...
Login To CMS And Delete User From CMS
    ...
    ${status}    ${result}    Run Keyword And Ignore Error   Wait Until Element Is Visible   ${deleted_msg}    timeout=20
    Run Keyword If    '${status}' == 'PASS'    Log    Mobile number deleted succesfully
    ...    ELSE     Close Window
    Switch Window    Main
...
Get values From CMS
    ...
    Wait Until Element Is Visible    ${showcase_icon}  timeout=30
    Click Element    ${showcase_icon}
    Wait Until Element Is Visible    ${download_data_link}      timeout=30
    Click Element    ${download_data_link}
    Wait Until Element Is Visible    ${select_option_dropdown}  timeout=30
    Click Element    ${select_option_dropdown}
    Select From List By Value    ${select_option_dropdown}  21
    Press Key    ${select_option_dropdown}    ENTER
    Wait For Five Seconds
    Wait Until Element Is Visible    ${start_date_timepicker}   timeout=30
    Click Element    ${start_date_timepicker}
    Sleep    20
#    Input Text    ${start_date_timepicker}    05/11/2024 07:03 PM
    Wait Until Element Is Visible    ${end_date_timepicker}     timeout=30
    Click Element    ${end_date_timepicker}
    Wait For Five Seconds
    Wait Until Element Is Visible   ${download_user_button}     timeout=30
    Double Click Element    ${download_user_button}